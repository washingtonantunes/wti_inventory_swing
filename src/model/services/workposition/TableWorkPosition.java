...
public class TableWorkPosition extends JTable {

	private static final long serialVersionUID = 1L;

	public TableWorkPosition(WorkPositionTableModel model) {
		super(model);
		initComponents();
	}
	...
	private void configureSizeColumn() {
		...
		for (int i = 0; i < this.getColumnCount(); i++) {
			...
			if (i == 0) {
				...
				column.setPreferredWidth(110); // Location
				column.setResizable(false);
			} 
			else if (i == 2) {
				column.setPreferredWidth(130); // Floor
				column.setResizable(false);
			} 
			else if (i == 3) {
