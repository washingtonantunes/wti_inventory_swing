...
public class EquipmentDaoJDBC implements EquipmentDao {
	...
	public void updateStatusForUser(Equipment obj) {
		...
		try {
			...
			st.setString(3, obj.getProject() != null ? obj.getUser().getProject().getCostCenter() : "");
			st.setString(4, obj.getUser() != null ? obj.getUser().getRegistration() : "");
	...
	public Map<String, Equipment> findAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				equipment.setProject(instatiateProject(rs));
