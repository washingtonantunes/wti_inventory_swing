...
public class DisableProjectForm extends JDialog {
	...
	public DisableProjectForm(ProjectTableModel model, Project project, List<Option> options, int lineSelected) {
		this.model = model;
	...
	private void addLabelsAndComboBoxes(JPanel panel) {
		...
		comboBox_Reason = new JComboBox<>(new Vector<>(
				options.stream().filter(o -> o.getType().equals("REASON-PROJECT") && o.getStatus().equals("ACTIVE"))
	...
	private class buttonSaveListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			try {
				...
				service.disable(project);
				model.updateProject(lineSelected, project);
				dispose();
				JOptionPane.showMessageDialog(rootPane, "Project successfully disabled", "Success disabling object", JOptionPane.INFORMATION_MESSAGE);
			} 
	...
	private Project getFormData() {
		...
		if (comboBox_Reason.getSelectedIndex() < 0 || comboBox_Reason.getSelectedItem() == null) {
			...
			project.setReason(comboBox_Reason.getSelectedItem().toString());
		}
